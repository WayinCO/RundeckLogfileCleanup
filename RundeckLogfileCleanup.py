...
def get_projects():
    ...
    try:
        ...
        print "Problem with project listing"
        print e.message
        raise e
...
def get_jobs_for_project(project_name):
    ...
    try:
        ...
        print "Problem with job listing"
        print e.message
        raise e
    return job_ids
...
def get_executions_for_job(job_id, page):
    ...
    try:
        ...
        r = requests.get(url,
                         params={
                             'max': PROPERTIES['PAGE_SIZE'],
                             'offset': page * PROPERTIES['PAGE_SIZE']
                         },
                         headers=HEADERS,
                         verify=False,
                         timeout=PROPERTIES['TIMEOUT'])
        ...
        print "Problem with execution listing"
        print e.message
        raise e
    return root
...
def delete_execution(execution_id):
    ...
    try:
        ...
        if PROPERTIES['VERBOSE']:
            print "            Deleted execution id {0} {1} {2}".format(
                execution_id, r.text, r)
    except Exception as e:
        print e.message
        raise e
...
def delete_test(execution_date):
    ...
    return ((TODAY - execution_date) > millis_in_one_day *
            PROPERTIES['MAXIMUM_DAYS'])
    ...
    for (jobid, jobname) in get_jobs_for_project(project):
        ...
        while (more and len(deleteable) < PROPERTIES['MAX_DELETE']):
            try:
                ...
                print "        Page {0} got {1} jobs".format(
                    page, execution_root.attrib['count'])
                page += 1
                deleteable += check_deletion(
                    get_execution_dates(execution_root))
                more = int(execution_root.attrib['count']) == PROPERTIES[
                    'PAGE_SIZE']
            except:
                print traceback.format_exc()
                print "Problem with executions {0} {1}".format(
                    execution_root, sys.exc_info()[0])
                more = False
        if (len(deleteable) > 0):
